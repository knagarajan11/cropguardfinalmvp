...
for image_number, item in enumerate(evaluation_images, start=1):
    ...
    # --------------------------------------------------------
    #
    # If generation used the entire token budget, the model may have been
    # cut off mid-reasoning, before ever stating its actual conclusion.
    # Flag this per-image so truncated rows can be told apart from rows
    # where the model just answered incorrectly.
    ...
    if truncated:
        print()
        print(
            f"WARNING: generation used the full {MAX_NEW_TOKENS}-token "
            "budget. Response may be truncated mid-reasoning, before the "
            "model's final answer. Consider raising MAX_NEW_TOKENS."
        )
    ...
    # --------------------------------------------------------
    #
    # Extraction runs on the isolated final-answer segment (text after any
    # reasoning scratchpad delimiter), not the raw response, so a
    # scratchpad mentioning multiple disease names in passing can't be
    # mistaken for the model's actual conclusion. The full raw response is
    # still stored in the CSV either way.

    final_answer_text = isolate_final_answer(response)

    predicted_disease = extract_predicted_disease(final_answer_text, truncated=truncated)
...
# confusion_matrix(..., labels=labels) silently drops any prediction that
# isn't one of these three -- e.g. "unknown" or "incomplete" from a model
# response that never committed to a class. Left as-is, a row's displayed
# counts would sum to less than its actual support, which looks like a
# bug rather than what it is: an off-label prediction. We map every
# off-label prediction to an explicit "other" bucket so every row's
# counts always add up to its true total, and the categories the model
# actually returned are visible for the confusion matrix specifically
# (this mapping is local to this display only -- y_pred itself, and every
# other metric computed from it, is untouched).
...
if any(p not in labels for p in y_pred):
    print()
    print(
        "Note: 'Other' covers predictions the model never resolved to one "
        "of the three known classes (e.g. 'unknown' or 'incomplete'). See "
        "Step 16 for which images these were and why."
    )
...
print(f"{'Image':40s} {'Actual':14s} {'Predicted':14s} {'Confidence':16s} {'Trunc':7s} {'Result':6s}")
print("-" * 110)

for row in results:
    result_text = "PASS" if row["correct"] else "FAIL"
    trunc_text = "YES" if row.get("truncated") else ""
    confidence_text = str(row.get("confidence", "unknown"))
    if len(confidence_text) > 15:
        confidence_text = confidence_text[:12] + "..."
    print(
        f"{row['image'][-40:]:40s} "
        f"{row['ground_truth'][:14]:14s} "
        f"{row['prediction'][:14]:14s} "
        f"{confidence_text:16s} "
        f"{trunc_text:7s} "
        f"{result_text:6s}"
    )
