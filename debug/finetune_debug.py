        ...
        if self.pp_enabled:
            ...
            with sync_ctx, train_ctx():
                ...
                if isinstance(self.loss_fn, FusedLinearCrossEntropy):
                    ...
                    # ============================================================
                    # CROPGUARD DEBUG
                    # Inspect batch immediately before model(**batch)
                    # ============================================================
                    if torch.distributed.is_initialized() and torch.distributed.get_rank() == 0:
                        print("\n" + "=" * 80, flush=True)
                        print("CROPGUARD: BATCH IMMEDIATELY BEFORE MODEL", flush=True)
                        print("=" * 80, flush=True)

                        for k, v in batch.items():
                            print(
                                f"{k}: type={type(v)}, "
                                f"shape={getattr(v, 'shape', 'NO_SHAPE')}",
                                flush=True
                            )

                        pv = batch.get("pixel_values")

                        print("\n---------- PIXEL VALUES ----------", flush=True)
                        print("type:", type(pv), flush=True)
                        print(
                            "is Tensor:",
                        ...
                        if isinstance(pv, torch.Tensor):
                            print("shape:", pv.shape, flush=True)
                            print("dtype:", pv.dtype, flush=True)
                            print("device:", pv.device, flush=True)
                            ...
                            for i, item in enumerate(pv[:10]):
                                print(
                                    f"  [{i}] "
                                    f"type={type(item)}, "
                                    f"shape={getattr(item, 'shape', 'NO_SHAPE')}",
                                    flush=True
                                )

                        print("=" * 80, flush=True)
                        print("END CROPGUARD DEBUG", flush=True)
                        print("=" * 80 + "\n", flush=True)
