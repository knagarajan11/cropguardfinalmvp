...
Analyze the provided crop leaf image and classify it as exactly ONE of:
- early blight
- late blight
- healthy

Return ONLY these two lines. Do not provide reasoning, symptoms, alternatives,
recommendations, or any other text.
...
def run_generation(inputs, label):
    ...
    try:
        ...
        print(f"[{label}] Generation completed in {inference_time:.2f}s "
              f"({generated_tokens.shape[1]} tokens" + (", TRUNCATED" if truncated else "") + ")")
...
for image_number, item in enumerate(evaluation_images, start=1):
    ...
    # Release per-image tensors before loading the next image.
    try:
        del inputs
    except Exception:
        pass
